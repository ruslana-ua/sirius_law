 ...
 */
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".process__slider")) new swiper_core_Swiper(".process__slider", {
                ...
                pagination: {
                    ...
                    renderBullet: function(index, className) {
                        var titleHtml = document.querySelector(".process__slide:nth-child(" + (index + 1) + ") .process__title--top").innerHTML;
                        return '<div class="' + className + '">' + titleHtml + "</div>";
                    },
            ...
            if (document.querySelector(".publications-block__slider")) new swiper_core_Swiper(".publications-block__slider", {
            ...
            var infoPriceSlider = document.querySelector(".info-price__slider");
            if (infoPriceSlider) {
                var swiper = null;
                ...
                function initSwiperIfNeeded() {
                    ...
                    if (screenWidth < 991.98 && !swiper) swiper = initSwiper(infoPriceSlider); else if (screenWidth > 991.98 && swiper) {
            ...
            if (document.querySelector(".more-cases__slider")) new swiper_core_Swiper(".more-cases__slider", {
                modules: [ Navigation, Mousewheel ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
        ...
        const searchMobInputs = document.querySelectorAll(".search-mob__input");
        if (searchMobInputs) searchMobInputs.forEach((searchMobInput => {
            const clearInputBtn = searchMobInput.closest(".search-mob__form").querySelector(".search-mob__clear");
            if (searchMobInput && clearInputBtn) {
                function clearInput() {
                    searchMobInput.value = "";
                    clearInputBtn.classList.remove("visible");
                }
                searchMobInput.addEventListener("input", (() => {
                    const inputValue = searchMobInput.value.trim().toLowerCase();
        ...
        const footerContactsLink = document.querySelector(".footer__contacts-link");
        ...
        function moveBlock() {
            var screenWidth = window.innerWidth;
            var popularServicesBody = document.querySelector(".popular-services__body");
            var popularServicesWrapper = document.querySelector(".popular-services__wrapper");
            var popularServicesRow = document.querySelector(".popular-services__row");
            if (popularServicesBody && popularServicesWrapper && popularServicesRow) if (screenWidth < 991.98) popularServicesWrapper.appendChild(popularServicesRow); else popularServicesBody.appendChild(popularServicesRow);
        }
        moveBlock();
        window.addEventListener("resize", (function() {
            moveBlock();
        ...
        function digitsCountersAnimate(digitsCounter) {
            ...
            const step = timestamp => {
                ...
                digitsCounter.innerHTML = typeof digitsCounter.dataset.digitsCounterFormat !== "undefined" ? functions_getDigFormat(value, format) : value;
        ...
        var processSlideCounts = document.querySelectorAll(".process__slide");
        processSlideCounts.forEach((function(slide, index) {
            var titleSpan = slide.querySelector(".process__title--top span");
            if (titleSpan) titleSpan.textContent = (index + 1).toString();
            var countBlock = slide.querySelector(".process__count");
            if (countBlock) countBlock.textContent = (index + 1).toString();
        }));
        function handleAdvantageBlocks() {
            var advantageBlocks = document.querySelectorAll(".advantages-more__body");
            if (window.innerWidth < 767.98) advantageBlocks.forEach((function(block) {
                var col1 = block.querySelector(".advantages-more__col-1");
                var col2 = block.querySelector(".advantages-more__col-2");
                if (col1 && col2) while (col2.children.length > 0) col1.appendChild(col2.children[0]);
            })); else advantageBlocks.forEach((function(block) {
                var col1 = block.querySelector(".advantages-more__col-1");
                var col2 = block.querySelector(".advantages-more__col-2");
                if (col1 && col2) {
                    var childrenToMove = col1.children.length - 2;
                    for (var i = 0; i < childrenToMove; i++) col2.appendChild(col1.lastElementChild);
                }
            }));
        }
        var advantagesWrapper = document.querySelector(".advantages-more__wrapper");
        if (advantagesWrapper) {
            window.addEventListener("resize", handleAdvantageBlocks);
            window.addEventListener("load", handleAdvantageBlocks);
        }
        function handlePublicationsBlocks() {
            var publicationWrapper = document.querySelector(".publications-block__wrapper");
            if (publicationWrapper) {
                var links = publicationWrapper.querySelectorAll(".publications-block__link");
                links.forEach((function(link) {
                    var title = link.querySelector(".publications-block__title");
                    if (window.innerWidth < 767.98) link.appendChild(title); else {
                        var mainBlock = link.querySelector(".publications-block__main");
                        mainBlock.insertBefore(title, mainBlock.firstChild);
                    }
                }));
            }
        }
        window.addEventListener("resize", handlePublicationsBlocks);
        window.addEventListener("load", handlePublicationsBlocks);
        document.addEventListener("DOMContentLoaded", (function() {
        ...
        window.addEventListener("load", (() => {
            ...
            if (formLabelUploadInput) {
                ...
                const fileshow = fileName => {
                    const uploadWrapperBoxCount = document.querySelectorAll(".file-upload__inner").length;
                    if (uploadWrapperBoxCount < 5) {
                        ...
                        uploadWrapperFile.innerHTML = fileName;
                ...
                const displayErrorType = errorMessage => {
